...
MarlinFormat::MarlinFormat(Board& board) {
    ...
    epSquare = stm | (board.enpassantSq() == Square::NO_SQ
                          ? 64
                          : (board.enpassantSq() ^ 7).index());
    ...
    while (occ) {
        ...
        int pti = (int)pt;
        pti |= (Bitboard::fromSquare(sq) & board.us(Color::WHITE)).empty()
                   ? 1ULL << 3
                   : 0;
...
struct GameEntry {
        ...
        double wdl;
        GameEntry(std::string f, std::vector<Move> m, std::vector<int> s,
                  double w) {
            fen = f;
            moves = m;
            scores = s;
...
void writeBuffer(std::ofstream& outFile, GameEntry& game) {
    ...
    for (int i = 0; i < game.moves.size(); i++) {
        ...
        outFile << test.getFen() << " | " << game.scores[i] << " | " << game.wdl
                << std::endl;
    }
    outFile.flush();
}
...
void writeViriformat(std::ofstream& outFile, ViriEntry& game) {
    int32_t nullbytes = 0;
    outFile.write(reinterpret_cast<const char*>(&game.header),
                  sizeof(game.header));
    outFile.write(reinterpret_cast<const char*>(game.scores.data()),
                  sizeof(ScoredMove) * game.scores.size());
...
uint16_t packMove(Move m) {
    ...
    if (m.typeOf() == Move::CASTLING) {
        ...
        flag =
            0b11'00'000000'000000 | (((uint16_t)m.promotionType() - 1) << 12);
    }
    move |= (uint16_t)(from);
    move |= (uint16_t)(to) << 6;
    move |= flag;
...
void runThread(int ti) {
    ...
    std::unique_ptr<Search::ThreadInfo> thread =
        std::make_unique<Search::ThreadInfo>(ThreadType::SECONDARY, TT,
                                             nullptr);
    ...
    for (int G = 1; G < 1'000'000; G++) {
        ...
        while (end.second == GameResult::NONE) {
            ...
            int eval = Search::iterativeDeepening(std::ref(board), *thread,
                                                  limit, nullptr);
        ...
        if (G % 50 == 0) {
            std::cout << "Thread: " << ti << " Total Games: " << G
                      << " Positions: " << poses << " Estimated speed "
                      << cached * 1000.0 / (double)timer.elapsed() << " pos/s"
                      << std::endl;
            timer.start();
            cached = 0;
        }
        if (G % GAMES_BUFFER == 0) {
            std::cout << "Thread: " << ti << " writing " << GAMES_BUFFER
                      << " games" << std::endl;
