    ...
    int search(int depth, int ply, int alpha, int beta, bool cutnode, Stack* ss, ThreadInfo& thread, Limit& limit) {
        ...
        while (!moveIsNull(move = picker.nextMove())) {
            ...
            if (!root && bestScore > GETTING_MATED) {
                ...
                futility = ss->staticEval + BNFP_DEPTH_SCALE() * lmrDepth + BNFP_MOVECOUNT_SCALE() * moveCount / 128;
                if (!inCheck && lmrDepth <= 5 && picker.stage == MPStage::BAD_NOISY && std::abs(alpha) < 2000 && futility <= alpha) {
